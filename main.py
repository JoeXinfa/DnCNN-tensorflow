...
def denoiser_train(denoiser, lr):
    ...
    eval_files = os.path.join(args.work_dir, 'data/test/{}/*.png'.format(args.eval_set))
    ...
    with load_data(filepath=data_file) as data:
        ...
        denoiser.train(data, eval_data, batch_size=batch_size, ckpt_dir=ckpt_dir, epoch=epoch, lr=lr,
                       sample_dir=sample_dir, log_dir=log_dir)


def denoiser_test(denoiser):
    test_files = os.path.join(args.work_dir, 'data/test/{}/*.png'.format(args.test_set))
    ckpt_dir = os.path.join(args.work_dir, args.ckpt_dir)
